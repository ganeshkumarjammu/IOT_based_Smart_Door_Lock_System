...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if ((text == "/lockstatus") || (text == "/lockstatus@Smart_Dora_bot" )) {
      String r_msg = "Door Lock Status :"+String(lockState) ;
      bot.sendMessage(CHAT_ID, r_msg, "");
    }
    if ((text == "/start") || (text == "/start@Smart_Dora_bot" )) {
      ...
      welcome += "/detect : To turn on/off Automatic detection\n";
    ...
    if ((text == "/detect") || (text == "/detect@Smart_Dora_bot" )) {
      msgEnabled = !msgEnabled ;
      if (msgEnabled) {
        String r_msg = "Automatic detection turned on";
        bot.sendMessage(CHAT_ID, r_msg, "");
      }
      else {
        String r_msg = "Automatic detection turned off";
        bot.sendMessage(CHAT_ID, r_msg, "");
      }
    }
    if ((text == "/buzz") || (text == "/buzz@Smart_Dora_bot" )) {
      buzzState = !buzzState ;
      digitalWrite(BUZZER, buzzState);
      if (buzzState) {
        String r_msg = "Buzzer turned on";
        bot.sendMessage(CHAT_ID, r_msg, "");
      }
      else {
        String r_msg = "Buzzer turned off";
        bot.sendMessage(CHAT_ID, r_msg, "");
      }
    }
  }
...
void loop() {
  ...
  Serial.println(" cm");
  delay(50);
  if (distance_cm < 50 && msgEnabled ) {
    String r_msg = "Someone/something is at the door.\n To get photo, type"; //whether this text will be received by the esp32cam ??
    bot.sendMessage(CHAT_ID, r_msg, "");
    r_msg = "/photo";
    bot.sendMessage(CHAT_ID, r_msg, "");
    r_msg = "";
